...
func MakeRecipeImplementation(querier supertokens.Querier) plessmodels.RecipeInterface {
	createCode := func(email *string, phoneNumber *string, userInputCode *string, tenantId *string, userContext supertokens.UserContext) (plessmodels.CreateCodeResponse, error) {
		...
		response, err := querier.SendPostRequest(supertokens.GetPathPrefixForTenantId(tenantId)+"/recipe/signinup/code", body)
		if err != nil {
	...
	consumeCode := func(userInput *plessmodels.UserInputCodeWithDeviceID, linkCode *string, preAuthSessionID string, tenantId *string, userContext supertokens.UserContext) (plessmodels.ConsumeCodeResponse, error) {
		...
		response, err := querier.SendPostRequest(supertokens.GetPathPrefixForTenantId(tenantId)+"/recipe/signinup/code/consume", body)
		if err != nil {
	...
	listCodesByDeviceID := func(deviceID string, tenantId *string, userContext supertokens.UserContext) (*plessmodels.DeviceType, error) {
		response, err := querier.SendGetRequest(supertokens.GetPathPrefixForTenantId(tenantId)+"/recipe/signinup/codes", map[string]string{
			"deviceId": deviceID,
		})
	...
	revokeAllCodes := func(email *string, phoneNumber *string, tenantId *string, userContext supertokens.UserContext) error {
		...
		_, err := querier.SendPostRequest(supertokens.GetPathPrefixForTenantId(tenantId)+"/recipe/signinup/codes/remove", body)
		if err != nil {
			return err
		}
		return nil
	...
	revokeCode := func(codeID string, tenantId *string, userContext supertokens.UserContext) error {
		...
		_, err := querier.SendPostRequest(supertokens.GetPathPrefixForTenantId(tenantId)+"/recipe/signinup/code/remove", body)
		if err != nil {
			return err
		}
		return nil
	...
	updateUser := func(userID string, email *string, phoneNumber *string, tenantId *string, userContext supertokens.UserContext) (plessmodels.UpdateUserResponse, error) {
		...
		response, err := querier.SendPutRequest(supertokens.GetPathPrefixForTenantId(tenantId)+"/recipe/user", body)
		if err != nil {
