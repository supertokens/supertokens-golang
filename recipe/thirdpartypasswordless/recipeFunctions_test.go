...
func TestWithThirdPartyPasswordlessForThirdPartyUserThatIsEmailVerifiedReturnsTheCorrectEmailVerificationStatus(t *testing.T) {
	...
	resp, err := ThirdPartyManuallyCreateOrUpdateUser("custom", "verifiedUser", "test@example.com", nil)
	...
	resp1, err := ThirdPartyManuallyCreateOrUpdateUser("custom2", "NotVerifiedUser", "test@example.com", nil)
	assert.NoError(t, err)
...
func TestWithThirdPartyPasswordlessForPasswordlessUserThatIsEmailVerifiedReturnsTrueForPhoneAndFalseForEmail(t *testing.T) {
	...
	response, err := PasswordlessSignInUpByEmail("test@example.com", nil)
	assert.NoError(t, err)
	...
	response, err = PasswordlessSignInUpByPhoneNumber("+123456789012", nil)
	assert.NoError(t, err)
...
func TestWithThirdPartyPasswordlessGetUserFunctionality(t *testing.T) {
	...
	resp, err := PasswordlessSignInUpByEmail("test@example.com", nil)
	assert.NoError(t, err)
	userId := resp.User.ID
	...
	users, err := GetUsersByEmail("random", nil)
	assert.NoError(t, err)
	assert.Equal(t, 0, len(users))

	users, err = GetUsersByEmail("test@example.com", nil)
	assert.NoError(t, err)
	...
	user, err = GetUserByPhoneNumber("random", nil)
	assert.NoError(t, err)
	assert.Nil(t, user)

	resp, err = PasswordlessSignInUpByPhoneNumber("+1234567890", nil)
	assert.NoError(t, err)

	user, err = GetUserByPhoneNumber(*resp.User.PhoneNumber, nil)
	assert.NoError(t, err)
	assert.NotNil(t, user)
...
func TestThirdPartyPasswordlessCreateNewCodeFromDevice(t *testing.T) {
	...
	resp1, err := CreateNewCodeForDevice(resp.OK.DeviceID, nil, nil)
	assert.NoError(t, err)
	...
	userInputCode := "2314"
	resp1, err = CreateNewCodeForDevice(resp.OK.DeviceID, &userInputCode, nil)
	assert.NoError(t, err)
	...
	resp1, err = CreateNewCodeForDevice("random", nil, nil)
	assert.NoError(t, err)
	assert.NotNil(t, resp1.RestartFlowError)
	assert.Nil(t, resp1.OK)

	resp, err = CreateCodeWithEmail("test@example.com", &userInputCode, nil)
	assert.NoError(t, err)
...
func TestThirdPartyPasswordlessConsumeCoed(t *testing.T) {
	...
	result, err := ConsumeCodeWithUserInputCode(resp.OK.DeviceID, resp.OK.UserInputCode, resp.OK.PreAuthSessionID, nil)
	assert.NoError(t, err)
	...
	result, err = ConsumeCodeWithUserInputCode(resp.OK.DeviceID, "random", resp.OK.PreAuthSessionID, nil)
	assert.NoError(t, err)
	...
	_, err = ConsumeCodeWithUserInputCode(resp.OK.DeviceID, resp.OK.UserInputCode, "random", nil)
...
func TestThirdPartyPasswordlessUpdateUserContactMethodEmailTest(t *testing.T) {
	...
	userInfo, err := PasswordlessSignInUpByEmail("test@example.com", nil)
	assert.NoError(t, err)

	updatedEmail := "test2@example.com"
	resp, err := UpdatePasswordlessUser(userInfo.User.ID, &updatedEmail, nil, nil)
	assert.NoError(t, err)
	assert.NotNil(t, resp.OK)
	...
	resp, err = UpdatePasswordlessUser("random", &updatedEmail, nil, nil)
	assert.NoError(t, err)
	assert.Nil(t, resp.OK)
	assert.NotNil(t, resp.UnknownUserIdError)

	userInfo2, err := PasswordlessSignInUpByEmail("test3@example.com", nil)
	assert.NoError(t, err)

	resp, err = UpdatePasswordlessUser(userInfo2.User.ID, &updatedEmail, nil, nil)
	assert.NoError(t, err)
	assert.Nil(t, resp.OK)
...
func TestThirdPartyPasswordlessUpdateUserContactPhone(t *testing.T) {
	...
	userInfo, err := PasswordlessSignInUpByPhoneNumber(phoneNumber_1, nil)
	assert.NoError(t, err)

	res1, err := UpdatePasswordlessUser(userInfo.User.ID, nil, &phoneNumber_2, nil)
	assert.NoError(t, err)
	...
	userInfo1, err := PasswordlessSignInUpByPhoneNumber(phoneNumber_3, nil)
	assert.NoError(t, err)

	res1, err = UpdatePasswordlessUser(userInfo1.User.ID, nil, &phoneNumber_2, nil)
	assert.NoError(t, err)
...
func TestThirdPartyPasswordlessListCodesByEmail(t *testing.T) {
	...
	result, err := ListCodesByEmail("test@example.com", nil)
	assert.NoError(t, err)
...
func TestListCodesByPhoneNumber(t *testing.T) {
	...
	codeInfo1, err := CreateCodeWithEmail("+1234567890", nil, nil)
	assert.NoError(t, err)

	codeInfo2, err := CreateCodeWithEmail("+1234567890", nil, nil)
	assert.NoError(t, err)

	result, err := ListCodesByEmail("+1234567890", nil)
	assert.NoError(t, err)
...
func TestThirdPartyPasswordlessListCodesByDeviceIdAndListCodesByPreAuthSessionId(t *testing.T) {
	...
	result, err := ListCodesByDeviceID(codeInfo1.OK.DeviceID, nil)
	assert.NoError(t, err)

	assert.Equal(t, codeInfo1.OK.CodeID, result.Codes[0].CodeID)

	result, err = ListCodesByPreAuthSessionID(codeInfo1.OK.PreAuthSessionID, nil)
	assert.NoError(t, err)
...
func TestCreateMagicLinkTest(t *testing.T) {
	...
	result, err := CreateMagicLinkByPhoneNumber("+1234567890", nil)
	assert.NoError(t, err)
...
func TestThirdPartyPasswordlessSignInUp(t *testing.T) {
	...
	result, err := PasswordlessSignInUpByPhoneNumber("+12345678901", nil)
	assert.NoError(t, err)
