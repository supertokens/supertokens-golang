...
func SignIn(apiImplementation webauthnmodels.APIInterface, tenantId string, options webauthnmodels.APIOptions, userContext supertokens.UserContext) error {
	...
	var credential webauthnmodels.AuthenticationPayload
	...
	resp, err := (*apiImplementation.SignInPOST)(optionsId, credential, nil, tenantId, options, userContext)
	if err != nil {
		return err
	}
	...
	if resp.CredentialNotFoundError != nil {
		return supertokens.Send200Response(options.Res, map[string]interface{}{"status": "CREDENTIAL_NOT_FOUND_ERROR"})
	}
	if resp.UnknownUserIdError != nil {
		return supertokens.Send200Response(options.Res, map[string]interface{}{"status": "UNKNOWN_USER_ID_ERROR"})
	}
