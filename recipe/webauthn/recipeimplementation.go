...
func MakeRecipeImplementation(querier supertokens.Querier, recipeEmailDelivery emaildelivery.Ingredient) webauthnmodels.RecipeInterface {
	...
	getUserByEmail := func(
		...
		usersRaw, ok := resp["users"].([]interface{})
		if !ok {
			return nil, nil
		}
		for _, u := range usersRaw {
			...
			for _, lm := range user.LoginMethods {
				if lm.RecipeID == "webauthn" && lm.Email != nil && *lm.Email == email {
					return &user, nil
				}
			}
		}
	...
	getUserFromRecoverAccountToken = func(
		...
		if err != nil {
			return webauthnmodels.GetUserFromRecoverAccountTokenResponse{}, err
		}
		status, _ := resp["status"].(string)
		if status == "RECOVER_ACCOUNT_TOKEN_INVALID_ERROR" {
			return webauthnmodels.GetUserFromRecoverAccountTokenResponse{
				RecoverAccountTokenInvalidError: &struct{}{},
			}, nil
		}
		if status != "OK" {
		...
		userMap, _ := resp["user"].(map[string]interface{})
		if userMap == nil {
			return webauthnmodels.GetUserFromRecoverAccountTokenResponse{},
				fmt.Errorf("missing user in recover-token response")
		}
		user := parseUser(userMap)
		var recipeUserId *string
		if r, ok := resp["recipeUserId"].(string); ok && r != "" {
			recipeUserId = &r
		}
		return webauthnmodels.GetUserFromRecoverAccountTokenResponse{
			OK: &struct {
				...
				RecipeUserId *string
			}{User: user, RecipeUserId: recipeUserId},
		}, nil
	}
