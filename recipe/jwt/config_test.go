...
func TestDefaultConfigSetsValuesCorrectlyForJWTRecipe(t *testing.T) {
	...
	jwtR, err := getRecipeInstanceOrThrowError()
	if err != nil {
		t.Error(err.Error())
	}

	assert.Equal(t, 3153600000, int(jwtR.Config.JwtValiditySeconds))
}
...
func TestConfigSetsValueCorrectlyForJWTrecipeWhenJWTValidityisSet(t *testing.T) {
	...
	configValue := supertokens.TypeInput{
		...
		RecipeList: []supertokens.Recipe{
			Init(&jwtmodels.TypeInput{
				JwtValiditySeconds: &customValidity,
			}),
		},
	}
	...
	assert.Equal(t, customValidity, jwtR.Config.JwtValiditySeconds)
}
