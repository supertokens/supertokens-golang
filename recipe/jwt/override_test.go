			...
			Init(&jwtmodels.TypeInput{
				Override: &jwtmodels.OverrideStruct{
					Functions: func(originalImplementation jwtmodels.RecipeInterface) jwtmodels.RecipeInterface {
						...
						*originalImplementation.CreateJWT = func(payload map[string]interface{}, validitySeconds *uint64, userContext supertokens.UserContext, useStaticSigningKey *bool) (jwtmodels.CreateJWTResponse, error) {
							resp, err := createJWToriginal(payload, validitySeconds, userContext, useStaticSigningKey)
							if err != nil {
								t.Error(err.Error())
