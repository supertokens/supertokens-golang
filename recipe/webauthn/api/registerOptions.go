...
func RegisterOptions(apiImplementation webauthnmodels.APIInterface, tenantId string, options webauthnmodels.APIOptions, userContext supertokens.UserContext) error {
	...
	if displayNameVal, ok := bodyMap["displayName"]; ok {
	...
	resp, err := (*apiImplementation.RegisterOptionsPOST)(email, recoverAccountToken, displayName, tenantId, options, userContext)
	if err != nil {
		return err
	}
