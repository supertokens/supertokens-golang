...
func MakeEmailPasswordRecipeImplementation(recipeImplementation tpepmodels.RecipeInterface) epmodels.RecipeInterface {
	...
	getUserByID := func(userId string, tenantId *string, userContext supertokens.UserContext) (*epmodels.User, error) {
		user, err := (*recipeImplementation.GetUserByID)(userId, tenantId, userContext)
		if err != nil {
			return nil, err
		}
	...
	updateEmailOrPassword := func(userId string, email, password *string, tenantId *string, userContext supertokens.UserContext) (epmodels.UpdateEmailOrPasswordResponse, error) {
		return (*recipeImplementation.UpdateEmailOrPassword)(userId, email, password, tenantId, userContext)
	}
