...
func RecoverAccount(apiImplementation webauthnmodels.APIInterface, tenantId string, options webauthnmodels.APIOptions, userContext supertokens.UserContext) error {
	if apiImplementation.RecoverAccountPOST == nil {
	...
	if !ok || token == "" {
		return supertokens.BadInputError{Msg: "token is required"}
	}
	...
	resp, err := (*apiImplementation.RecoverAccountPOST)(optionsId, credential, token, tenantId, options, userContext)
	if err != nil {
		return err
	}
	...
	if resp.RecoverAccountTokenInvalidError != nil {
		return supertokens.Send200Response(options.Res, map[string]interface{}{"status": "RECOVER_ACCOUNT_TOKEN_INVALID_ERROR"})
	}
